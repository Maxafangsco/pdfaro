...
test.describe('Homepage', () => {
  ...
  test('hero search navigates to /en/tools with query', async ({ page }) => {
    ...
    // Next.js routes to /en/tools?q=merge (or /en/tools/?q=merge with trailing slash).
    // The [/?]+ pattern handles both the path separator and the query string ? character.
...
// Correct slugs as defined in src/app/[locale]/tools/[tool]/page.tsx
const TOOL_SLUGS = [
  'merge-pdf',
  'compress-pdf',
  'split-pdf',
  'rotate-pdf',
  'image-to-pdf',
  'encrypt-pdf',    // correct slug (not 'protect-pdf')
  'add-watermark',  // correct slug (not 'watermark-pdf')
  'pdf-to-jpg',     // correct slug (not 'pdf-to-image')
  'sign-pdf',
  'edit-pdf',
];
...
for (const slug of TOOL_SLUGS) {
  test(`/en/tools/${slug} loads without errors`, async ({ page }) => {
    ...
    // Page should render — tool-page data-testid is set by ToolPage component
    await expect(page.getByTestId('tool-page')).toBeVisible();

    // No unhandled JS errors (ResizeObserver is a known benign browser noise)
